...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $parent = $db->prepare("SELECT * FROM parents WHERE id=?");
    $parent->execute([$pid]);
    $parent = $parent->fetch();

    if ($parent['password'] && !verifyPassword($current, $parent['password'])) {
        ...
        $db->prepare("UPDATE parents SET password=? WHERE id=?")->execute([hashPassword($new), $pid]);
